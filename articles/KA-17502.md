...
<br><br>Struttura predefinita della cartella Enterprise Linux<br><br><br><br><br><br>
In AMS l’installazione di base utilizza Enterprise Linux come sistema operativo di base.  Quando installi Apache Webserver, avrà un file di installazione predefinito impostato.  Ecco i file predefiniti che vengono installati installando gli RPM di base forniti dall&#39;archivio yum
...
- /etc/httpd/conf.d/enabled_vhosts/
   - Quando sei pronto a utilizzare i file .vhost che hai all&#39;interno della cartella available_vhosts, puoi collegarli mediante un percorso relativo nella directory enabled_vhosts

<br><br><br><br> <br><br>Ulteriori directory conf.d<br><br><br><br> <br><br>
Ci sono altre parti comuni nelle configurazioni di Apache e abbiamo creato sottodirectory per consentire un modo pulito di separare quei file e non avere tutti i file in una directory

<b>Directory di riscrittura</b>

Questa directory può contenere tutti i file \*_rewrite.rules creati che contengono i file tipici *RewriteRule* sintassi che coinvolga i server web Apache [mod_rewrite](https://httpd.apache.org/docs/current/mod/mod_rewrite.html) modulo
...
Questa directory può contenere tutti i file \*_whitelist.rules creati che contengono i file tipici *Consenti IP* o *Richiedi IP* sintassi che coinvolga i server web Apache [controlli di accesso](https://httpd.apache.org/docs/2.4/howto/access.html)
...
Questa directory può contenere tutti i file \*.vars creati che contengono le variabili utilizzabili nei file di configurazione
...
<br><br><br><br> <br><br>Ulteriori directory conf.dispatcher.d<br><br><br><br> <br><br>
...
Questa directory può contenere tutti i file \*_clientheaders.any creati che contengono elenchi di intestazioni client da passare a AEM quando arriva una richiesta.  Maggiori dettagli su questa sezione sono [qui](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders)
...
Questa directory può contenere tutti i file \*_filters.any creati che contengono tutte le regole di filtro per bloccare o consentire al traffico attraverso il dispatcher di raggiungere AEM
...
Questa directory può contenere tutti i file \*_renders.any creati che contengono i dettagli di connettività a ciascun server di back-end da cui il dispatcher utilizzerà il contenuto
...
Questa directory può contenere tutti i file \*_vhosts.any creati che contengono un elenco dei nomi di dominio e dei percorsi da abbinare a una particolare farm a un particolare server back end
...
Viene invece creato un file .rpmnew accanto all’originale.  Questo significa che perderai alcune modifiche che avresti voluto e creato più rifiuti nelle cartelle di configurazione.

Enterprise Linux è stato configurato correttamente per gestire meglio questo caso d&#39;uso.  Forniscono aree in cui puoi estendere o sovrascrivere i valori predefiniti impostati per te.  All&#39;interno dell&#39;installazione di base di httpd troverai il file /etc/httpd/conf/httpd.conf, che contiene sintassi come:
...
<br><br><br><br><br> <br><br>
L’idea è che Apache desideri estendere i moduli e le configurazioni nell’aggiunta di nuovi file al<b> /etc/httpd/conf.d </b>e <b>/etc/httpd/conf.modules.d/</b> directory con estensione di file <b>.conf</b>

Come esempio perfetto quando si aggiunge il modulo dispatcher ad Apache si crea un file .so modulo in <b>/etc/httpd/modules/</b> e quindi includerlo aggiungendo un file in <b>/etc/httpd/conf.modules.d/02-dispatcher.conf</b> con il contenuto per caricare il file .so del modulo
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
<b>Avviso</b> non abbiamo modificato alcun file già esistente fornito da Apache.  Invece abbiamo semplicemente aggiunto le nostre alle directory che dovevano seguire.
