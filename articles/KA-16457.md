...
Osserva SegmentNotFoundException nei file di log AEM e AEM non funziona come previsto

O

L’esecuzione di una compattazione offline può non riuscire con SegmentNotFoundException quando si verificano problemi di integrità dell’archivio. Una traccia di stack simile a quella riportata di seguito si trova nei log:
...
L&#39;esecuzione di una compattazione offline può non riuscire con IllegalArgument Exception quando ci sono problemi di integrità dell&#39;archivio. Una traccia di stack simile a quella riportata di seguito si trova nei log:
...
Un SegmentNotFoundException viene restituito quando un segmento non è presente mentre la compattazione tenta di leggere il nodo. Ci possono essere diverse cause principali per questo:

1. Il segmento è stato rimosso con un intervento manuale (ad es. `rm -rf /`).
...
Nel caso in cui il problema sia causato dalla revisione della raccolta oggetti inattivi (punto 2), assicurati che la compattazione online sia disabilitata per evitare che altri nodi vengano danneggiati.
...
*Attenzione:<b>* </b>Questo processo consente di ripristinare i dati del sistema in un momento precedente.  Se desideri evitare di perdere le modifiche nel sistema, puoi provare l&#39;opzione B qui sotto.

Per eseguire il controllo e il ripristino:

1. Scarica il file jar oak-run da qui [http://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/](http://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/)
2. AEM.
...
4. Ripristina l&#39;archivio a questa revisione tramite modifica.

   `/crx-quickstart/repository/segmentstore/journal.log.`



   Elimina tutte le righe dopo la riga contenente l&#39;ultima revisione valida. Se desideri sapere in che data e ora stai ripristinando l&#39;archivio, esegui questo comando nella cartella segmentstore (sostituisci `afdb922d-ba53-4a1b-aa1b-1cb044b535cf` con l&#39;ultima buona revisione nel tuo `journal.log`):
...
6. Se utilizzi AEM6.0, scarica la versione oak-run corrispondente a quella installata in AEM per i passaggi rimanenti.  Scarica da qui [http://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/](http://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/).
...
*Attenzione:*La procedura seguente è destinata agli utenti dell&#39;alimentazione.  Quando si eliminano i nodi corrotti è necessario assicurarsi che non siano nodi di sistema (ad esempio `/home`, `/jcr:system`, ecc.) .  Oppure, se si tratta di nodi di sistema, è necessario assicurarsi di poterli ripristinare.  Se non sei sicuro, consulta AEM team di Assistenza clienti per assistenza sui passaggi qui documentati.
...
4. Ripetere il passaggio 3 per tutti i nodi trovati al passaggio 2.
   ...
   Questo qui sopra `rmNode` Il comando deve restituire true per il percorso corrotto, il che significa che lo ha eliminato. Assicurati che questi tre percorsi corrotti trovati vengano eliminati eseguendo nuovamente il `rmNode` su questi percorsi. L&#39;esecuzione successiva deve restituire false.
