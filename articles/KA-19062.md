---
title: "Eseguire il debug di SegmentNotFoundException quando viene segnalato un problema in AEM 6.x"
...
Innanzitutto, utilizza lo strumento oak run, un file jar1 eseguibile che contiene tutto ciò che è necessario per una semplice installazione di Oak e per l’esecuzione di operazioni relative a Oak.
...
*Attenzione: questo processo consente di eseguire il rollback dei dati del sistema a un momento precedente.  Se vuoi evitare di perdere le modifiche nel sistema, puoi provare l’opzione B riportata di seguito.*
...
3. Esegui la verifica di coerenza, java -Xmx6000m -jar oak-run-\*.jar check —bin=-1 /path/to/crx-quickstart/repository/segmentstore Questa ricerca all&#39;indietro attraverso le revisioni fino a trovarne una coerente: Cerca un messaggio simile al seguente:
...
1. Rimuovere tutti i nodi danneggiati identificati elencati nell&#39;output dell&#39;ultimo comando tramite rmNodes.groovy


Carica la shell della console oak-run utilizzando il comando seguente:
...
dove il percorso del nodo corrotto è il percorso ottenuto nel passaggio 2, ad esempio: &quot;/content/dam/test.txt/jcr:content/renditions/original/jcr:content/&quot; Nota: quando utilizzi oak-run.jar versione 1.6.13 e successive, imposta —parametro JVM di lettura-scrittura se riscontri un errore, ad esempio:
...
1. Esegui una pulizia dei punti di controllo elencandoli tramite il comando seguente. Se sono presenti più punti di controllo, effettua la pulizia:

   ```
   nohup java -Xmx4096m -jar oak-run-1.2.18.jar checkpoints /app/AEM6/author/crx-quickstart/repository/segmentstore rm-allnohup.out &
   ```
2. Esegui una compattazione offline.  Se non sai come eseguire la compattazione offline, consulta [qui](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b).
3. Avvia il server e attendi il completamento dell’indicizzazione.
