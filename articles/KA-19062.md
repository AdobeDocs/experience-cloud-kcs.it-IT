...
La modalità di esecuzione del controllo di [!DNL oak-run] può essere utilizzato per determinare l&#39;ultima revisione valida nota di un archivio segmenti.  Può essere utilizzato per ripristinare manualmente un archivio di segmenti corrotti alla sua ultima revisione valida.
...
1. Esegui il controllo di coerenza,
...
1. Ripristina l&#39;archivio a questa revisione modificando `./crx-quickstart/repository/segmentstore/journal.log` e l&#39;eliminazione di tutte le righe dopo la riga contenente l&#39;ultima revisione valida.

1. Rimuovi tutto `./crx-quickstart/repository/segmentstore/\*.bak` file.
...
1. Rimuovi tutti i nodi danneggiati identificati elencati nell&#39;output dell&#39;ultimo comando utilizzando `rmNodes.groovy`
...
1. Ripetere il passaggio 3 per tutti i nodi trovati al passaggio 2.

   Questo qui sopra `rmNode` Il comando deve restituire true per il percorso corrotto, il che significa che lo ha eliminato. Assicurati che questi tre percorsi corrotti trovati vengano eliminati eseguendo nuovamente il comando rmNode su quei percorsi. L&#39;esecuzione successiva deve restituire false.
...
1. Esegui una pulizia dei checkpoint elencando i checkpoint utilizzando di seguito. Se sono presenti più punti di controllo, puliscili:
...
1. Avvia il server in attesa del completamento dell&#39;indicizzazione.
