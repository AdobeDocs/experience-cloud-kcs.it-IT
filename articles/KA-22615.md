---
title: "Problemi di configurazione di GraphQl AEM"
...
# Problemi di configurazione di GraphQl AEM
...
Il cliente sta sviluppando un sito web che dovrebbe recuperare i dati dalle proprie istanze AEM utilizzando la soluzione headless AEM.

Per impostare l’endpoint, l’esercitazione in `[` 1`]`  è stato seguito.

Dopo aver pubblicato tutti i frammenti di contenuto, tutti i modelli, il contenuto, i riferimenti, le risorse e le query come descritto in `[` 2`]` , quando si tenta di recuperare un frammento di contenuto dall’istanza di pubblicazione, viene visualizzato un errore 404 che impedisce la lettura del contenuto da parte dell’editore.
...
`[` 1`]`  aem_base_version: 6.5 aem_envId: xxxxxxx aem_envTipo: dev aem_program_id: xxxxxx aem_release_id: xxxxx aem_service: cm-pxxxxxx-exxxxxxx aem_tenant: ns-team-aem-cm-prd-nxxxx aem_tier: publish backend: publishfarm/- cache: <b>bloccato</b>
cluster: `<` cluster`>`
codice: - file_path: /var/log/aem/dispatcher.log host: publish-pxxxxxx-exxxxxxx.adobeaemcloud.com metodo: POST namespace: ns-team-aem-cm-prd-nxxxxxx orig_time: 25/lug/2023:15:39:11 +0000 percorso: /content/cq:graphql/graphql/endpoint.json nome_pod: `<` pod_name`>`
pod_uid: `<` pod_uid`>`
proto: HTTP proto_version: 1.1 tempo_risposta: 0
