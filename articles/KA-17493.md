...
[&lt;- Precedente: Uso e nozioni di base sulle variabili](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17487.html)
...
![l’autore attiva il contenuto, il che induce l’editore a inviare una richiesta di scaricamento al dispatcher](assets/dispatcher-flushing-order-of-events.png "ordine degli eventi del dispatcher-flushing")
...
![schermata dell’agente di replica standard dalla pagina web AEM /etc/replication.html](assets/author-rep-agent-example.png "author-rep-agent-example")
...
![schermata dell&#39;agente di replica di scaricamento standard dalla pagina web AEM /etc/replication.html](assets/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")

### REPLICA FLUSH DEL DISPATCHER CHE RICEVE UN HOST VIRTUALE

Il modulo dispatcher cerca intestazioni particolari per sapere quando una richiesta di POST è qualcosa da trasmettere a AEM render o se è serializzata come richiesta di scaricamento e deve essere gestita dal gestore del dispatcher stesso.

Ecco una schermata della pagina di configurazione che mostra questi valori:
![immagine della scheda delle impostazioni della schermata di configurazione principale con il tipo di serializzazione mostrato come Dispatcher Flush](assets/disp-flush-agent1.png "disp-flush-agent1")
...
![Schermata della scheda di trasporto dell’agente di replica.  Mostra l’URI a cui inviare la richiesta di scaricamento.  /dispatcher/invalidate.cache](assets/disp-flush-agent2.png "disp-flush-agent2")

Sulla `Transport` è possibile visualizzare la scheda `URI` impostato per puntare l’indirizzo IP del dispatcher che riceverà le richieste di scaricamento.  Il percorso `/dispatcher/invalidate.cache` non è il modo in cui il modulo determina se si tratta di uno scaricamento, è solo un endpoint ovvio che puoi vedere nel log di accesso per sapere che si tratta di una richiesta di scaricamento.  Sulla `Extended` esamineremo gli elementi presenti per qualificare che si tratta di una richiesta di scaricamento al modulo dispatcher.

![Schermata della scheda Extended dell’agente di replica.  Prendi nota delle intestazioni che vengono inviate con la richiesta POST inviata per comunicare al dispatcher di eseguire il flush](assets/disp-flush-agent3.png "disp-flush-agent3")
...
![Schermata di un agente di replica standard che mostra che l’agente di replica reagisce e si attiva quando nuovi elementi sono stati ricevuti da un evento di replica dall’autore che pubblica i contenuti](assets/disp-flush-agent4.png "disp-flush-agent4")
...
![file di stato misti con contenuto e date mostrati con i livelli di stato mostrati](assets/dispatcher-statfiles.png "dispatcher-statfiles")
...
[Successivo -> URL personalizzati](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17463.html)