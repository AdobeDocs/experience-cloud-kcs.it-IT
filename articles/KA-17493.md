...
<br><br><br><br><br><br>AGENTE DI REPLICA DELL&#39;AUTORE<br><br><br><br><br><br>
...
La pagina delle impostazioni predefinite mostra la <b>[!UICONTROL Tipo di serializzazione]</b> come *[!UICONTROL Flush del Dispatcher]* e imposta il livello di errore
...
Sulla [!UICONTROL Trasporti] è possibile visualizzare la scheda [!UICONTROL URI] impostato per puntare l’indirizzo IP del dispatcher che riceverà le richieste di scaricamento.  Il percorso `/dispatcher/invalidate.cache` non è il modo in cui il modulo determina se si tratta di uno scaricamento, è solo un endpoint ovvio che puoi vedere nel log di accesso per sapere che si tratta di una richiesta di scaricamento.  Sulla [!UICONTROL Esteso] esamineremo gli elementi presenti per qualificare che si tratta di una richiesta di scaricamento al modulo dispatcher.
...
La [!UICONTROL metodo HTTP] per le richieste di scaricamento è solo una richiesta di GET con alcune intestazioni di richiesta speciali:
...
- Host
   - Qui è dove `Host` L’intestazione viene modificata in modo da eseguire il targeting di uno specifico `VirtualHost` configurato sul dispatcher [!DNL Apache] server web (`/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost`).  È un valore hardcoded che corrisponde a una voce nel `aem_flush.vhost` file *`ServerName`* o *`ServerAlias`*
...
Sulla [!UICONTROL Triggers] prenderemo nota dei trigger attivati che usiamo e di cosa sono

- [!UICONTROL Ignora valori predefiniti]
...
- [!UICONTROL Nessun controllo delle versioni]
...
<br><br><br><br><br><br>La `.stat` file<br><br><br><br><br><br>
...
Se si dovesse eseguire il seguente comando sul server dispatcher, si inizierebbe a trovare `.stat` file
...
![file di stato misti con contenuto e date mostrati con i livelli di stato mostrati](https://helpx.adobe.com/content/dam/help/en/experience-manager/kb/ams-dispatcher-manual/disp-flushing/jcr_content/main-pars/image_2035486865/dispatcher-statfiles.png "dispatcher-statfiles")<br><br><br><br><br><br>LIVELLO DI FILE STAT<br><br><br><br>
Notate che in ogni directory c&#39;era un `.stat` file presente.  Questo è un indicatore che si è verificato uno scaricamento.  Nell’esempio sopra il <b>`statfilelevel`</b> impostazione impostata su <b>`3`</b> all’interno del file di configurazione della farm corrispondente.

La `statfilelevel` l’impostazione indica quante cartelle in fondo al modulo attraversano e aggiornano un `.stat` file.  Il file .stat è vuoto, non è altro che un nome file con un datestamp e potrebbe anche essere creato manualmente ma eseguendo il comando touch sulla riga di comando del server dispatcher.
...
<br><br><br><br> <br><br>HANDSHAKE A MARCA TEMPORALE FILE STATO<br><br><br><br><br><br>
...
<br><br><br><br><br><br>GESTIONE CACHE - ESEMPIO 1<br><br><br><br><br><br>
...
<br><br><br><br><br><br>GESTIONE CACHE - ESEMPIO 2<br><br><br><br><br><br>
...
<br><br><br><br><br><br>Impostazioni dei file farm<br><br><br><br><br><br>
...
<br><br><br><br><br><br>Radice documento<br><br><br><br><br><br>
...
Questa directory deve corrispondere al [!DNL Apache] impostazione della directory principale dei documenti per il dominio configurato per l&#39;utilizzo dal server Web.

Avere cartelle docroot nidificate per ogni farm che ospita una sottocartella del [!DNL Apache] document root è un&#39;idea terribile per molti motivi.


<br><br><br><br><br><br>Livello file di stato<br><br><br><br><br><br>
Questa voce di configurazione si trova nella seguente sezione del file farm:
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br><br>Invalidazione consentita<br><br><br><br><br><br>
Questa voce di configurazione si trova nella seguente sezione del file farm:
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br><br><br>Regole di invalidazione<br><br><br><br><br><br>
Questa voce di configurazione si trova nella seguente sezione del file farm:
<br><br><br><br><br> <br><br><br><br>
...
<br><br>Test / Risoluzione dei problemi<br><br><br><br><br><br>
...
Una volta licenziato il comando di richiesta al dispatcher, dovrai vedere cosa viene fatto nei log e cosa viene fatto con il `.stat files`.  Esegui il tail del file di log e dovresti vedere le seguenti voci per confermare che la richiesta di scaricamento ha colpito il modulo dispatcher
<br><br><br><br><br> <br><br><br><br>
