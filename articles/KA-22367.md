...
Dopo aver creato la configurazione SAML 2.0 per abilitare l’accesso SSO nell’istanza di authoring di produzione, il cliente ha ricevuto un messaggio di errore dal lato AEM dopo che il provider di Azure ha eseguito l’accesso e ha recuperato il token SAML per eseguire l’autenticazione sul lato AEM.<br><br> <br><br>L’errore ricevuto è stato `[` 1`]` <br><br> <br><br><b>`[` 1`]`  \*ERRORE\* `[` qtp1558388453-153215`]`  Il parametro di risposta SAML com.adobe.granite.auth.saml.SamlAuthenticationHandler non è stato fornito o non è valido.</b>
...
L’errore 403 in genere viene visualizzato se il filtro Referrer Apache Sling non è abilitato. Controllo della configurazione per il filtro Referrer Apache Sling: l’opzione &quot;Allow empty&quot; (Consenti vuoto) non era selezionata e non era stato aggiunto alcun host in &quot;Allow Hosts&quot; (Consenti host)
...
Dopo aver completato questi passaggi richiesti: seleziona &quot;Allow Empty&quot; (Consenti vuoto) e l’opzione Add the hosts that Representative the IdP provider to the &quot;Allow Hosts&quot; (Aggiungi host). L’accesso è riuscito.



NOTA:

L’URL IDP deve essere aggiunto alla configurazione del filtro di riferimento Sling Apache senza includere il protocollo, ad esempio aem-sso-saml invece di https://aem-sso-saml
