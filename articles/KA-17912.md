...
Questo documento illustra come avviene il caching del dispatcher e come può essere configurato<br><br> <br><br>[Sommario](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17490.html)
...
Dispatcher controlla cosa si qualifica come memorizzabile in cache nella sezione /cache { di qualsiasi file farm. 
...
Ecco una sezione dell’autore base /cache { del file farm dell’autore:
...
Assicurati che il DocumentRoot nel file .vhost dell&#39;autore corrisponda al parametro farms /docroot
...
Questa è una delle gemme di una caratteristica del dispatcher. Se l’editore è sotto carico o non risponde, in genere genera un codice di risposta http 502 o 503. Se questo accade e questa funzione è abilitata, il dispatcher sarà istruito a continuare a servire quale contenuto è ancora nella cache come miglior sforzo, anche se non è una nuova copia. È meglio servire qualcosa se ce l&#39;hai invece di mostrare un messaggio di errore che non offre funzionalità.
...
Se hai pagine in AEM che accettano argomenti o parametri di query di GET nella riga dell&#39;indirizzo che aiutano il funzionamento della pagina ma non esegue il rendering di html diversi sono buoni candidati per questo elemento di configurazione Puoi dire al dispatcher quali argomenti ignorare e comunque memorizzare la pagina nella cache.
...
Ora, quando il dispatcher vede la richiesta, ignorerà il fatto che la richiesta ha il parametro di query di ?reference e memorizza ancora in cache la pagina



<b>Memorizzazione in cache delle intestazioni di risposta</b>

È abbastanza ovvio che il dispatcher memorizza in cache le pagine .html e clientlibs, ma sapevi che può anche memorizzare in cache particolari intestazioni di risposta lungo il contenuto in un file con lo stesso nome ma con estensione .h. Questo consente la risposta successiva non solo al contenuto, ma anche alle intestazioni di risposta che dovrebbero accompagnarlo dalla cache.
...
Questi valori quando vengono memorizzati nella cache vengono rimossi per impostazione predefinita e il webserver apache httpd eseguirà il proprio lavoro di elaborazione della risorsa con i suoi normali metodi di gestione dei file, che normalmente si limitano all’indovinazione dei tipi mime in base alle estensioni dei file.

Se disponi della cache del dispatcher la risorsa e le intestazioni desiderate puoi esporre l&#39;esperienza corretta e garantire che tutti i dettagli lo facciano al browser dei client.
...
Se hai 5 richieste di annullare la validità di /content/exampleco/en/, tutto avviene entro un periodo di 3 secondi.
...
Tieni presente che AEM deve ancora essere configurato per inviare intestazioni TTL per il dispatcher per rispettarle. L’attivazione di questa funzione consente solo al dispatcher di sapere quando rimuovere i file per i quali AEM invia intestazioni di controllo cache. Se AEM non inizia a inviare intestazioni TTL, il dispatcher non farà nulla di speciale qui.
