---
title: "Esempi di analisi request.log | AEP"
...
# Esempi di analisi request.log | AEP
...
Adobe Experience Manager (AEM) `request.log` contiene varie informazioni utili, ad esempio il tempo di risposta, per l’analisi dei problemi di prestazioni. Ecco un elenco di esempi di analisi che utilizzano i comandi Linux (inclusi alcuni comandi esterni).
...
4. Elaborazione simultanea massima
5. Dividere un file di registro
6. Unisci record di richiesta e record di risposta
...
5. Minimo, medio (medio), mediano, tempo massimo di risposta
6. Numero di accessi per periodo
7. Numero di stati di risposta per un periodo
8. URL più frequenti
9. record access.log per un record request.log
