...
  
I dettagli interni suggeriscono un problema di risposta HTTP con un messaggio che indica &quot;Not Found&quot; (Non trovato), che in genere implica che l’endpoint per l’istanza di authoring non è raggiungibile durante l’esecuzione del test.
...
Tuttavia, l’utente cloudmanager esiste e il caricamento manuale delle risorse sull’autore funziona come previsto.

Il test delle prestazioni delle risorse funziona per 30 minuti, quindi improvvisamente si interrompe in modo ripetuto, mentre dovrebbe terminare dopo 30 minuti. `[` 1`]`
...
Durante la verifica dei registri interni della funzione di Azure è stata trovata la richiesta con risultato 404 Not Found.

Il GET all’URL dal campo &quot;message&quot; ha restituito 404 `[` 1`]` .

L’errore in questo caso è stato causato dal fatto che, seguendo le raccomandazioni di Adobe, querybuilder è stato bloccato a livello di dispatcher.

Tuttavia, Cloud Manager deve poter utilizzare querybuilder nel dispatcher per l’utente di cloudmanager, in quanto durante il test delle risorse recupera il tempo di elaborazione.
